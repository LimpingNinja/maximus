...
static char rcs_id[]="$Id: mr.c,v 1.4 2004/01/27 23:02:55 paltas Exp $";
...
static void near renumber_area(const char *sys_path, PMAH pmah)
{
  char path[PATHLEN];
  word num_msg;
  REN *ren;

  if (maxcfg_resolve_path(sys_path, PMAS(pmah, path), path, sizeof(path)) != MAXCFG_OK)
    return;
...
static int near scan_adat(char *adat, char **astart, const char *sys_path)
{
  HAF haf;
  HAFF haff;
  MAH mah={0};
...
int _stdc main(int argc, char **argv)
{
...

  {
    ...
    if (maxcfg_toml_get(cfg, "maximus.sys_path", &v) == MAXCFG_OK && v.type == MAXCFG_VAR_STRING)
      sys_path = v.v.s;
    if (maxcfg_toml_get(cfg, "maximus.message_data", &v) == MAXCFG_OK && v.type == MAXCFG_VAR_STRING)
      message_data = v.v.s;

    if (!sys_path || !*sys_path || !message_data || !*message_data)
    {
      printf("Error: Missing required keys in maximus.toml (need maximus.sys_path and maximus.message_data)\n");
      return 1;
    }

    if (maxcfg_resolve_path(sys_path, message_data, adat, sizeof(adat)) != MAXCFG_OK)
    {
      printf("Error: message_data path too long\n");
      return 1;
    }
  }
  ...
  ret=scan_adat(adat, argv+1, sys_path);
