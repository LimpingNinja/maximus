...
static void near ScanOneArea(struct _sbcfg *psc, PMAH pmah)
{
  ...
  if (maxcfg_resolve_path(psc->sys_path, PMAS(pmah, path), pszMsgpath, sizeof(pszMsgpath)) != MAXCFG_OK)
    return;
...
static void near Init(struct _sbcfg *psc, int argc, char **argv)
{
...

  {
    ...
    if (maxcfg_toml_init(&psc->cfg) != MAXCFG_OK)
      NoMem();

    if (maxcfg_toml_load_file(psc->cfg, maximus_toml, "maximus") != MAXCFG_OK)
    {
      printf("Error: Failed to load %s\n", maximus_toml);
      exit(1);
    }

    if (maxcfg_toml_get(psc->cfg, "maximus.sys_path", &v) == MAXCFG_OK && v.type == MAXCFG_VAR_STRING)
      sys_path = v.v.s;
    if (maxcfg_toml_get(psc->cfg, "maximus.file_password", &v) == MAXCFG_OK && v.type == MAXCFG_VAR_STRING)
      user_file = v.v.s;
    if (maxcfg_toml_get(psc->cfg, "maximus.message_data", &v) == MAXCFG_OK && v.type == MAXCFG_VAR_STRING)
      message_data = v.v.s;

    if (!sys_path || !*sys_path || !user_file || !*user_file || !message_data || !*message_data)
    {
      printf("Error: Missing required keys in maximus.toml (need maximus.sys_path, maximus.file_password, maximus.message_data)\n");
      exit(1);
    }

    psc->sys_path = sys_path;

    if (maxcfg_resolve_path(sys_path, user_file, psc->userindex, sizeof(psc->userindex)) != MAXCFG_OK)
      exit(1);
    if (maxcfg_resolve_path(sys_path, message_data, psc->mareafile, sizeof(psc->mareafile)) != MAXCFG_OK)
      exit(1);
  }
}

static void near Term(struct _sbcfg *psc)
{
  MsgCloseApi();

  if (psc && psc->cfg) {
    maxcfg_toml_free(psc->cfg);
    psc->cfg = NULL;
    psc->sys_path = NULL;
  }
}
