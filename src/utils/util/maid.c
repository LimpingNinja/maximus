...
static char toml_lang_path[PATHLEN];
static char err=FALSE;
static char firststring=TRUE;
static unsigned longest=0;

static char write_toml_meta=FALSE;
static char p_arg[PATHLEN];
static char current_langfile[PATHLEN];

static unsigned int maid_max_ptrs=0;
static unsigned int maid_max_heap=0;
static unsigned int maid_max_glh_ptrs=0;
static unsigned int maid_max_glh_len=0;
static unsigned int maid_max_syh_ptrs=0;
static unsigned int maid_max_syh_len=0;
...
static void Write_Heap(void)
{
  ...
  strncpy(current_langfile, langfile, sizeof(current_langfile) - 1);
  current_langfile[sizeof(current_langfile) - 1] = '\0';
  ...
  maid_max_ptrs = (unsigned int)ginf.max_gptrs_len;
  maid_max_heap = (unsigned int)ginf.max_gheap_len;
  maid_max_glh_ptrs = (unsigned int)((h[0].ns + h[0].as) * (int)sizeof(HOFS));
  maid_max_glh_len = (unsigned int)(h[1].heapstart - h[0].heapstart);
  maid_max_syh_ptrs = 0;
  maid_max_syh_len = 0;
  if (nh >= 3)
  {
    maid_max_syh_ptrs = (unsigned int)((h[1].ns + h[1].as) * (int)sizeof(HOFS));
    maid_max_syh_len = (unsigned int)(h[2].heapstart - h[1].heapstart);
  }
...
static int Load_Toml_Lang_Path(const char *base_dir)
{
  MaxCfg *cfg = NULL;
  MaxCfgToml *toml = NULL;
  char maximus_toml[PATHLEN];
  MaxCfgVar v;
  ...
  if (maxcfg_open(&cfg, base_dir) != MAXCFG_OK || cfg == NULL)
    goto done;

  if (maxcfg_toml_init(&toml) != MAXCFG_OK || toml == NULL)
    goto done;

  if (maxcfg_join_path(cfg, "config/maximus.toml", maximus_toml, sizeof(maximus_toml)) != MAXCFG_OK)
    goto done;

  if (maxcfg_toml_load_file(toml, maximus_toml, "maximus") != MAXCFG_OK)
    goto done;

  if (maxcfg_toml_get(toml, "maximus.lang_path", &v) != MAXCFG_OK || v.type != MAXCFG_VAR_STRING || v.v.s == NULL)
    goto done;

  if (strlen(v.v.s) >= sizeof(toml_lang_path))
    goto done;

  strcpy(toml_lang_path, v.v.s);
  ...
  return (*toml_lang_path != '\0') ? TRUE : FALSE;
}

static void Compile_Language(const char *base_dir)
{
  iname[0]=iiname;
  ...
  if ((ifile[0]=fopen(iiname,"r"))==NULL)
  {
    ...
    if (toml_lang_path[1] != ':' && toml_lang_path[0] != '\\' && toml_lang_path[0] != '/')
    {
      strcpy(temp, base_dir ? base_dir : "");
      strcat(temp, toml_lang_path);
    }
    else
      strcpy(temp, toml_lang_path);
...
static void Maid_Format(void)
{
  ...
  printf("   -p[PATH]  Update config/general/language.toml sizing fields.  If PATH is\n"
         "             provided and points to etc/max (or etc/max.ctl), it will be used\n"
         "             to locate the install root.  Otherwise MAXIMUS/MAX_INSTALL_PATH\n"
         "             is used.\n");
  exit(1);
}
  ...
  for (x=1;x < argc;x++)
  {
    if (*argv[x]=='/' || *argv[x]=='-')
    {
      switch(argv[x][1])
      {
        ...
        case 'p':
          write_toml_meta=TRUE;
          if (argv[x][2] != '\0')
            strcpy(p_arg, argv[x]+2);
          break;
...
static const char *ResolveBaseDir(char *out, size_t out_sz)
{
  const char *s;
  ...
  if (*p_arg)
  {
    ...
    strncpy(tmp, p_arg, sizeof(tmp) - 1);
    tmp[sizeof(tmp) - 1] = '\0';
    ...
    {
      char *leaf = p1 + 1;
      if (eqstri(leaf, "max") || eqstri(leaf, "max.ctl") || eqstri(leaf, "max.prm"))
      {
        *p1 = '\0';
        p1 = strrchr(tmp, '/');
        p2 = strrchr(tmp, '\\');
...
static int Update_Language_Toml(const char *base_dir, const char *langfile)
{
  MaxCfg *cfg = NULL;
  MaxCfgToml *toml = NULL;
  char language_toml[PATHLEN];
  MaxCfgVar v;
  const char *sysop_lang = NULL;
  ...
  if (maxcfg_join_path(cfg, "config/general/language.toml", language_toml, sizeof(language_toml)) != MAXCFG_OK)
    goto fail;

  if (maxcfg_toml_init(&toml) != MAXCFG_OK || toml == NULL)
    goto fail;

  if (maxcfg_toml_load_file(toml, language_toml, "language") != MAXCFG_OK)
    goto fail;

  (void)maxcfg_toml_override_set_uint(toml, "language.max_ptrs", maid_max_ptrs);
  (void)maxcfg_toml_override_set_uint(toml, "language.max_heap", maid_max_heap);

  {
    (void)maxcfg_toml_override_set_uint(toml, "language.max_glh_ptrs", maid_max_glh_ptrs);
    (void)maxcfg_toml_override_set_uint(toml, "language.max_glh_len", maid_max_glh_len);
  }

  if (maxcfg_toml_get(toml, "language.lang_file", &v) == MAXCFG_OK && v.type == MAXCFG_VAR_STRING_ARRAY)
  {
    if (v.v.strv.count > 0)
      sysop_lang = v.v.strv.items[0];
  }

  if (sysop_lang && langfile && *langfile && eqstri(sysop_lang, langfile))
  {
    (void)maxcfg_toml_override_set_uint(toml, "language.max_syh_ptrs", maid_max_syh_ptrs);
    (void)maxcfg_toml_override_set_uint(toml, "language.max_syh_len", maid_max_syh_len);
  }

  if (maxcfg_toml_persist_overrides_and_save(toml) != MAXCFG_OK)
    goto fail;
...
int _stdc main(int argc,char *argv[])
{
  ...
  pszBase = ResolveBaseDir(base_dir, sizeof(base_dir));
  if (pszBase)
    Load_Toml_Lang_Path(pszBase);
  ...
  Compile_Language(pszBase ? pszBase : "");
  ...
  {
    if (pszBase)
      (void)Update_Language_Toml(pszBase, current_langfile);
  }
