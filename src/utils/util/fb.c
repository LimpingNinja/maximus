...
int _stdc main(int argc, char *argv[])
{
  ...
  char *pszMaximus;           /* Maximus install/config root */
...

  {
    ...
    if (maxcfg_resolve_path(pszMaximus, "config/maximus", maximus_toml, sizeof(maximus_toml)) != MAXCFG_OK)
    {
      printf("Error: Could not resolve config/maximus from '%s'\n", pszMaximus);
      return EXIT_FAILURE;
    }
    ...
    if (maxcfg_toml_load_file(cfg, maximus_toml, "maximus") != MAXCFG_OK)
    {
      printf("Error: Failed to load %s\n", maximus_toml);
      return EXIT_FAILURE;
    }

    if (maxcfg_toml_get(cfg, "maximus.sys_path", &v) == MAXCFG_OK && v.type == MAXCFG_VAR_STRING)
      sys_path = v.v.s;
    if (maxcfg_toml_get(cfg, "maximus.file_data", &v) == MAXCFG_OK && v.type == MAXCFG_VAR_STRING)
      file_data = v.v.s;

    if (!sys_path || !*sys_path || !file_data || !*file_data)
    {
      printf("Error: Missing required keys in maximus.toml (need maximus.sys_path and maximus.file_data)\n");
      return EXIT_FAILURE;
    }

    /* Default file area data base name (no extension) */
    if (maxcfg_resolve_path(sys_path, file_data, adat_name, sizeof(adat_name)) != MAXCFG_OK)
    {
      printf("Error: file_data path too long\n");
      return EXIT_FAILURE;
    }

    /* maxfiles.idx is always in the system path */
    if (maxcfg_resolve_path(sys_path, "maxfiles.idx", master_idx, sizeof(master_idx)) != MAXCFG_OK)
    {
      printf("Error: sys_path too long\n");
      return EXIT_FAILURE;
    }
  }
  ...
  for (anum=0; AreaFileFindNext(haff, &fah, FALSE)==0; anum++)
  {
    ...
    if (fah.fa.filesbbs)
    {
      MaxCfgVar v;
      const char *sys_path = NULL;
      if (cfg && maxcfg_toml_get(cfg, "maximus.sys_path", &v) == MAXCFG_OK && v.type == MAXCFG_VAR_STRING)
        sys_path = v.v.s;
      if (!sys_path)
        sys_path = pszMaximus;

      maxcfg_resolve_path(sys_path, FAS(fah, filesbbs), szFilesBbs, sizeof(szFilesBbs));
    }
    else
    {
      MaxCfgVar v;
      ...
      maxcfg_resolve_path(sys_path, FAS(fah, downpath), szFilesBbs, sizeof(szFilesBbs));
    ...
    if (cAreas==0 || do_this_area(&fah, ppszAreas, fDoSlow))
    {
      ...
      if (fDoSlow || (fah.fa.attribs & FA_SLOW)==0)
      {
        if (fUp)
        {
          ...
          if (!sys_path)
            sys_path = pszMaximus;
          maxcfg_resolve_path(sys_path, FAS(fah, uppath), temp, sizeof(temp));
...

        {
          MaxCfgVar v;
          const char *sys_path = NULL;
          const char *rel = fUp ? FAS(fah, uppath) : FAS(fah, downpath);
          if (cfg && maxcfg_toml_get(cfg, "maximus.sys_path", &v) == MAXCFG_OK && v.type == MAXCFG_VAR_STRING)
            sys_path = v.v.s;
          if (!sys_path)
            sys_path = pszMaximus;
          maxcfg_resolve_path(sys_path, rel, szFilePath, sizeof(szFilePath));
        }
