...
static RAW_DISPLAY_STATE raw_display_state = { FALSE, -1L };

static sword near DisplayRaw(DSTK *d, RAW_DISPLAY_STATE *raw);
static void near DisplayDetectSauce(DSTK *d, RAW_DISPLAY_STATE *raw);
...
static sword near DisplayOneFile(DSTK *d)
{
  ...
  *last_onexit='\0';
  ...
  if (raw_display_state.active)
    ret=DisplayRaw(d, &raw_display_state);
...
static sword near DisplayRaw(DSTK *d, RAW_DISPLAY_STATE *raw)
{
  ...
  /* Reset raw display state after display completes */
  raw->active=FALSE;
  raw->raw_end=-1L;
...
static void near DisplayDetectSauce(DSTK *d, RAW_DISPLAY_STATE *raw)
{
  ...
  if (d==NULL || d->bbsfile==-1 || raw==NULL)
    return;

  save_pos=tell(d->bbsfile);
  end_pos=lseek(d->bbsfile, 0L, SEEK_END);
  if (end_pos < 128L)
    goto done;

  sauce_pos=end_pos-128L;
  if (lseek(d->bbsfile, sauce_pos, SEEK_SET)==-1L)
    goto done;

  if (read(d->bbsfile, (char *)rec, (unsigned)sizeof(rec)) != (int)sizeof(rec))
    goto done;

  if (memcmp(rec, "SAUCE", 5) != 0)
    goto done;
  ...
  if (comments)
  {
    comnt_len=5L + ((long)comments * 64L);
    comnt_pos=sauce_pos - comnt_len;
    if (comnt_pos >= 0)
    {
      if (lseek(d->bbsfile, comnt_pos, SEEK_SET) != -1L &&
          read(d->bbsfile, (char *)hdr, (unsigned)sizeof(hdr)) == (int)sizeof(hdr) &&
          memcmp(hdr, "COMNT", 5) == 0)
        cut_pos=comnt_pos;
    }
  }
  ...
  if (cut_pos > 0)
  {
    if (lseek(d->bbsfile, cut_pos-1L, SEEK_SET) != -1L &&
        read(d->bbsfile, (char *)&b, 1) == 1 &&
        b==0x1a)
      cut_pos--;
  }
  ...
  lseek(d->bbsfile, save_pos, SEEK_SET);
}
...
{
  unsigned want=FILEBUFSIZ;
  long pos;
  long remaining;
  sword got;

  if (raw_display_state.active && raw_display_state.raw_end >= 0L)
  {
    pos=tell(d->bbsfile);
    if (pos < 0)
      return DISP_EOF;
    if (pos >= raw_display_state.raw_end)
      return DISP_EOF;

    remaining=raw_display_state.raw_end - pos;
    if (remaining <= 0)
      return DISP_EOF;
    if (remaining < (long)want)
      want=(unsigned)remaining;
  }

  got=read(d->bbsfile, d->filebufr, want);
