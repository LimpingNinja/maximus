...
static char rcs_id[]="$Id: disp_max.c,v 1.4 2004/01/27 21:00:26 paltas Exp $";
  ...
  switch (i)
  {
    case MSGKEY_LATTACH:
      if (*ngcfg_get_string_raw("general.session.attach_base") &&
          (*ngcfg_get_string_raw("general.session.attach_path") || MAS(*pmah, attachpath)))
        priv = ngcfg_get_int("general.session.msg_localattach_priv");
      break;
...
word DisplayMaxCode(DSTK *d)
{
  ...
  switch (DispSlowGetChar(d))
  {
    ...
    case '\x03':
      Puts((char *)ngcfg_get_string_raw("maximus.system_name"));
      break;
      
    case '\x04':
      Puts((char *)ngcfg_get_string_raw("maximus.sysop"));
      break;
    ...
    case 'u':
      vbuf_flush();
      DispGetToBlank(d, d->scratch);
      play_tune((char *)ngcfg_get_path("general.display_files.tune"), d->scratch, wait_for_it,
