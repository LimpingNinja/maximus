...
static char rcs_id[]="$Id: t_misc.c,v 1.5 2004/01/28 06:38:11 paltas Exp $";
...
static void near GetExcludeList(void)
{
  FILE *fp;
  char line[PATHLEN];
  const char *name = ngcfg_get_path("general.session.track_exclude");
...
int TrackAreWeOwner(char *kludges, TRK_MSG_NDX *ptmn, int *pupdate_status)
{
  ...
  p=(char *)ngcfg_get_string_raw("general.session.track_privview");
                             ...
                             int *pupdate_status)
{
  ...
  if (TrkLookupMsg(t, actrack, NULL, NULL, NULL, &tmn))
  {
    ...
    if (PrivOK((char *)ngcfg_get_string_raw("general.session.track_privview"), FALSE))
      rc=TRUE;
  }
