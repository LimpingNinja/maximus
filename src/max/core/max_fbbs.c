...
static char rcs_id[]="$Id: max_fbbs.c,v 1.5 2004/01/28 06:38:10 paltas Exp $";
      ...
      else if (d->type & DISPLAY_SEARCH)
      {
        ...
        else
        {
              ...
              (p=stristr_nochin(line,searchfor)) != NULL)
          {
            ...
            if ((ngcfg_get_charset_int() & CHARSET_CHINESE)==0)
            {
              cstrlwr(filename);
              cstrlwr(line);
            }
  ...
  while (*desc)
  {
    ...
    *dw='\0';
    ...
    if ((word)(current_col+stravtlen(descwork)) > (word)TermWidth())
    {
      ...
      {
        int margin = ngcfg_get_int("general.session.filelist_margin");
        int ds = ngcfg_get_int("general.session.date_style");
        if (margin)
          Printf("\x19 %c", margin);
        else Printf("\x19 %c",  (ds==3 ? '\x1d' :
                              (ds==-1 ? '\x0e' : '\x1f'))-
                              (manualdate(*pfah) ? 1 : 0));
      }
