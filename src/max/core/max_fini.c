...
void FinishUp2(int hangup)
{
  ...
  /* Clean up node temp directory on disconnect */
  Clean_Node_Temp_Dir();
  ...
  if ((fLoggedOn || this_logon_bad) && !nowrite_lastuser && !in_wfc)
  {
    ...
    if (fLoggedOn && !acsflag(CFLAGA_HIDDEN))
    {
      ...
      if (ngcfg_get_bool("general.session.alias_system") && *usr.alias)
        name=usr.alias;
...
/*    LogWrite("\n");*/
  ...
  if (fossil_initd && rst_offset==-1L)
  {
    if (!local)
    {
      ...
      {
        int max_baud = ngcfg_get_int("general.equipment.baud_maximum");
        mdm_baud((char)Decimal_Baud_To_Mask((unsigned int)max_baud));
      }
      mdm_cmd((char *)ngcfg_get_string_raw("general.equipment.busy"));
    }
...
static void near ChatOff(void)
{
  ...
  sprintf(temp, ipcxx_bbs, ipc_base, task_num);
  unlink(temp);
}
...
/*  Restore_Directories();*/
  ...
  if (fossil_initd && rst_offset==-1L)
  {
    if (!local && !in_wfc)
    {
      {
        int max_baud = ngcfg_get_int("general.equipment.baud_maximum");
        mdm_baud(Decimal_Baud_To_Mask((unsigned int)max_baud));
      }
      mdm_cmd((char *)ngcfg_get_string_raw("general.equipment.busy"));
    }
...
void ShutDownVideo(void)
{
  ...
  if (!local && ngcfg_get_bool("maximus.status_line"))
...
{             /* only if erl > 0. (Typically used for an ERROR exit
  ...
  int echo_exit = ngcfg_get_int("matrix.after_echomail_exit");
  int edit_exit = ngcfg_get_int("matrix.after_edit_exit");
  int local_exit = ngcfg_get_int("matrix.after_local_exit");
  int exit_val = ngcfg_get_int("general.session.exit_after_call");

  if (el)
    erl=(char)el;
  else if (!fLoggedOn)
    erl=ERROR_RECYCLE;
  else if ((written_echomail || written_conf) && echo_exit)
    erl=(char)echo_exit;
  else if (written_matrix && edit_exit)
    erl=(char)edit_exit;
  else if (written_local && local_exit)
    erl=(char)local_exit;
  else if (exit_val)  /* If we have a specific el for after each caller */
    erl=(char)exit_val;
