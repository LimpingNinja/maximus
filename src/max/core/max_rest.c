...
static char rcs_id[]="$Id: max_rest.c,v 1.4 2004/01/28 06:38:10 paltas Exp $";
...
/*  memcpy(echo_written_in, rst->echo_written_in, sizeof(echo_written_in));*/
  ...
  if (ngcfg_get_bool("maximus.reboot") && !local)
    mdm_watchdog(0);
...
void Write_Restart(char restart_type, char *restart_name, long restart_offset, int usrtime, int ctltype, char *parm, long starttime, struct _css *css, char *returning)
{
  ...
  strcpy(rst->restart_name, restart_name ? restart_name : (char *)ngcfg_get_string_raw("general.session.first_menu"));
