            ...
            char *restart_name)
{
  ...
  if ((temp=malloc(PATHLEN*32))==NULL ||
      (temp2=malloc(PATHLEN*32))==NULL ||
      (args=malloc((MAX_EXECARGS+1)*sizeof(char *)))==NULL ||
  ...
  memset(args, 0, (MAX_EXECARGS+1) * sizeof(char *));
  ...
  /* Door32 keeps its flag on UNIX - we need to preserve the session FD */
#endif
...
/*  if (!in_wfc)
  ...
  /* Write all dropfiles to node temp directory */
...
/*      if ((prm.flags & FLAG_watchdog) && !local && !in_wfc)
  ...
  if (method==OUTSIDE_RUN || method==OUTSIDE_CHAIN ||
      method==OUTSIDE_CONCUR || method==OUTSIDE_DOOR32)
  {

    /***********************************************************************/
    /*                     RUN/CHAIN/DOOR32 EXIT                           */
    /***********************************************************************/
    ...
    logit("@Outside: about to Close_Files for '%s'", args[0]);
    Close_Files();
...
/*    if ((prm.flags & FLAG_watchdog) && !local && !in_wfc)
    ...
    logit("@Outside: about to IoPause for '%s'", args[0]);
    IoPause();
  ...
  #else
      logit("@Outside: about to xxspawnvp for '%s' method=%d", args[0], method);
      erl=xxspawnvp(P_WAIT, args[0], args, (method == OUTSIDE_DOOR32));
  #endif
    IoResume();
...
static void near Out_Save_Directories(int stay)
{
  ...
  if (ng_cfg &&
      maxcfg_toml_get(ng_cfg, "general.session.save_directories", &v) == MAXCFG_OK &&
      ...
      maxcfg_var_count(&v, &cnt) == MAXCFG_OK)
  {
    for (x=0; x < max_d; x++)
    {
      ...
      {
        MaxCfgVar it;
        if (maxcfg_toml_array_get(&v, i, &it) == MAXCFG_OK && it.type == MAXCFG_VAR_STRING && it.v.s && it.v.s[0] == letter)
          goto have_saved_drive;
      }
    }
    goto done_scan;
  }
...
static void near Close_Files(void)
{
  if (!ngcfg_get_bool("maximus.dos_close"))
  {
    LogFlush();
    return;
  }
...
void Shell_To_Dos(void)
{
  ...
  if (*ngcfg_get_path("general.display_files.shell_to_dos") && !in_wfc)
    Display_File(0, NULL, ngcfg_get_path("general.display_files.shell_to_dos"));
  ...
  if (*ngcfg_get_path("general.display_files.back_from_dos") && !in_wfc)
    Display_File(0, NULL, ngcfg_get_path("general.display_files.back_from_dos"));
...
int Exec_Xtern(int type, struct _opt *thisopt, char *arg, char **result, char *menuname)
{
  ...
  *result=NULL;
  ...
  logit("@Exec_Xtern: type=%d arg='%s'", type, arg ? arg : "(null)");
  ...
  switch(type)
  {
    ...
    case xtern_door32:  method=OUTSIDE_DOOR32;      break;
  ...
  logit("@Exec_Xtern: about to call Outside method=%d ctl=%d", method, ctl);
  ret=Outside((char *)ngcfg_get_path("general.display_files.out_leaving"),
              (char *)ngcfg_get_path("general.display_files.out_return"),
              method,
              Strip_Underscore(arg), TRUE, ctl, RESTART_MENU,
              menuname);
  logit("@Exec_Xtern: Outside returned %d", ret);
...
static void near Write_External_Ctlfile(int ctltype, char *parm, int method)
{
  ...
  if (!local)
  {
    int hs = ngcfg_get_handshake_mask_int();
    int cm = ngcfg_get_int("general.equipment.carrier_mask");
