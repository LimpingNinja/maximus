...
static void near WFCMaxBaud(void)
{
  ...
  if (max_baud <= 0)
    max_baud = startbaud ? (int)startbaud : 0;
  mdm_baud(Decimal_Baud_To_Mask(startbaud ? (unsigned int)startbaud : (unsigned int)max_baud));
}
...
static int near Process_Modem_Response(char *rsp)
{
  ...
  if (eqstri(rsp, ngcfg_get_string_raw("general.equipment.ring")))
  {
    const char *m_answer = ngcfg_get_string_raw("general.equipment.answer");

    if (*m_answer)
      mdm_cmd((char *)m_answer);
  }
  else
  {
    const char *m_connect = ngcfg_get_string_raw("general.equipment.connect");

    if (!eqstrni(rsp, m_connect, strlen(m_connect)))
      return 0;
...
static void near WFC_Init_Modem(void)
{
  ...
  mdm_cmd((char *)ngcfg_get_string_raw("general.equipment.init"));
  mdm_dtr(DTR_UP);
}
