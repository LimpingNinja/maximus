...
{
  {
    const char *menu = ngcfg_get_string_raw("general.session.edit_menu");
    return Display_Options((menu && *menu) ? menu : "EDIT", msg);
  }
}
...
int Bored(XMSG *msg,HMSG msgh,struct _replyp *pr)
{
  ...
  else                /* Things look A-OK! */
  {
    if (setjmp(jumpto)==0) /* Really ugly, but the best way to handle errs */
    {
      ...
      if (usr.help==NOVICE)
        Display_File(0, NULL, (char *)ngcfg_get_path("general.display_files.line_ed_help"));
...
static void near Bored_Edit(void)
{
  ...
  if (cx > 0 && cx <= num_lines)
  {
    ...
    if (usr.priv==NOVICE)
      Display_File(0, NULL, (char *)ngcfg_get_path("general.display_files.replace_help"));
...
/*static*/ word BodyToMsg(byte *txt, void *info, word lt)
{
  ...
  if ((lt & MSGLINE_SEENBY) && ! GEPriv(usr.priv, ngcfg_get_int("matrix.seenby_priv")))
    return TRUE;
  else if (lt & MSGLINE_KLUDGE && ! GEPriv(usr.priv, ngcfg_get_int("matrix.ctla_priv")))
    return TRUE;
...
word Local_Editor(XMSG *msg, HMSG msgh, long msgnum, char *ctrl_buf,struct _replyp *pr)
{
  ...
  sprintf(msgtemp, msgtemp_name, (char *)ngcfg_get_path("maximus.temp_path"), task_num);
  ...
  ed=(byte *)ngcfg_get_string_raw("general.session.local_editor");
