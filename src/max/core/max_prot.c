...
static char rcs_id[]="$Id: max_prot.c,v 1.4 2004/01/28 06:38:10 paltas Exp $";
...
word FindProtocol(sword protocol, struct _proto *pro)
{
  ...
  p=(char *)ngcfg_get_path("general.protocol.protocol_max_path");

  if (p && *p)
    strcpy(fname, p);
  else sprintf(fname, "%sprotocol.max", ngcfg_get_path("maximus.sys_path"));
