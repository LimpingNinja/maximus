...
static char rcs_id[]="$Id: log.c,v 1.3 2004/01/27 21:00:30 paltas Exp $";
...
void cdecl logit(char *format,...)
{
  ...
  switch(*p)
  {
    ...
    case '+':
      if (ngcfg_get_log_mode_int() >= 1)
        doit=TRUE;
      break;

    case '=':
      if (ngcfg_get_log_mode_int() >= 2)
        doit=TRUE;
      break;

    case ':':
      if (ngcfg_get_log_mode_int() >= 3)
        doit=TRUE;
      break;

    case '~':
      if (ngcfg_get_log_mode_int() >= 4)
        doit=TRUE;
      break;

    case '#':
      if (ngcfg_get_log_mode_int() >= 5)
        doit=TRUE;
      break;

    case '$':
      if (ngcfg_get_log_mode_int() >= 6)
        doit=TRUE;
      break;

    case ' ':     /* Never log these lines */
      if (ngcfg_get_log_mode_int() >= 7)
        doit=TRUE;
      break;
