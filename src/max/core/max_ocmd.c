 ...
 *   - Enhanced %P/%p to return file descriptor on UNIX for Door32.sys support
...
static char rcs_id[]="$Id: max_ocmd.c,v 1.4 2004/01/28 06:38:10 paltas Exp $";
 ...
 *  %p -- Port number (0=COM1, 1=COM2, etc.) or socket handle on UNIX      *
 *  %P -- Port number (1=COM1, 2=COM2, etc.) or socket handle on UNIX      *
 ...
 *  %# -- Node-specific temp directory path (e.g., /tmp/NODE1)            *
|*  %! -- Translates to a '\n' newline                                     *
        ...
        switch (*in)
        {
          ...
          case 'P':
            ...
            else
            {
              ...
              #elif defined(UNIX)
                /* On UNIX, return file descriptor for Door32 socket handle */
                sprintf(out, pd, FileHandle_fromCommHandle(ComGetHandle(hcModem)));
              #else
          ...
          case 's':
            strcpy(out,(p=firstchar((char *)ngcfg_get_string_raw("maximus.sysop"),ctl_delim,2))==NULL ?
                       no_last : p);
            break;
          ...
          case 'N':
            strcpy(out, (char *)ngcfg_get_string_raw("maximus.system_name"));
            break;
          ...
          case 'S':
            getword((char *)ngcfg_get_string_raw("maximus.sysop"), temp, ctl_delim, 1);
            strcpy(out, temp);
            break;
          ...
          case '#':
            /* Node-specific temp directory path */
            sprintf(out, "%snode%02x", (char *)ngcfg_get_path("maximus.temp_path"), (int)task_num);
            break;
