...
static char rcs_id[]="$Id: max_chat.c,v 1.4 2004/01/28 06:38:10 paltas Exp $";
...
long scan_time=NORM_SCAN_TIME;
      ...
      else
      {
        ...
        else
        {
          ...
          sprintf(psz,
                  ". %s %d server\0",
                  szMcpPipe,
                  ngcfg_get_int("maximus.mcp_sessions"));
  ...
  int ChatOpenIPC(byte tid)
  {
    ...
    sprintf(fname, ipcxx_bbs, ipc_base, tid);
    ...
    else if (!ngcfg_get_bool("maximus.no_share"))
    {
