...
static char rcs_id[]="$Id: events.c,v 1.4 2004/01/27 21:00:27 paltas Exp $";
...
static void near Write_Events(int e_num, int dateit)
{
  ...
  sprintf(edatn, event_name, ngcfg_get_path("maximus.sys_path"), e_num, dotdat);
  ...
  sprintf(ebbsn, event_name, ngcfg_get_path("maximus.sys_path"), e_num, dotbbs);
...
void Read_Event_File(int e_num)
{
  ...
  sprintf(edatname, event_name, ngcfg_get_path("maximus.sys_path"), e_num, dotdat);
