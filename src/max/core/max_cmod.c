...
static char rcs_id[]="$Id: max_cmod.c,v 1.5 2004/01/28 06:38:10 paltas Exp $";
...
void ChatMode(void)
{
  ...
  chatprog = (char *)ngcfg_get_string("general.session.chat_program");
  if (chatprog && *chatprog)
  {
  ...
  else
  {
    ...
    *lastword='\0';
    ...
    if (*ngcfg_get_path("general.display_files.chat_begin"))
      Display_File(0, NULL, ngcfg_get_path("general.display_files.chat_begin"));
    ...
    if (ngcfg_get_bool("general.session.chat_capture"))
      Open_Chatlog();
...
realEscape:
    ...
    if (local || carrier())
    {
      if (*ngcfg_get_path("general.display_files.chat_end"))
        Display_File(0, NULL, ngcfg_get_path("general.display_files.chat_end"));
      else Puts(chat_end);
    }
  }
...
{
  int cur = ngcfg_get_bool("general.session.yell_enabled");
  int newv = !cur;

  if (ng_cfg != NULL)
  {
    (void)maxcfg_toml_override_set_bool(ng_cfg, "general.session.yell_enabled", newv ? 1 : 0);
    (void)maxcfg_toml_persist_override_and_save(ng_cfg, "general.session.yell_enabled");
  }

  LocalMsg(yell_toggle, newv ? sys_on : sys_off);
}
