...
void Parse_FKey(int c)
{
  ...
  {
    size_t base_len = strlen(fkfile);
    char fkfile_vm[PATHLEN];
    char fkfile_bbs[PATHLEN];

    snprintf(fkfile_vm, sizeof(fkfile_vm), "%s.vm", fkfile);
    if (fexist(fkfile_vm))
    {
    ...
    {
      snprintf(fkfile_bbs, sizeof(fkfile_bbs), "%s.?bs", fkfile);
      if (fexist(fkfile_bbs))
      {
        display_line=display_col=1;
