...
# endif
  ...
  void Draw_StatusLine(int clearit)
  {
    ...
    if (! (displaymode==VIDEO_IBM && !local && ngcfg_get_bool("maximus.status_line")))
      return;
  ...
  int Highbit_Allowed(void)
  {
    ...
    if (ngcfg_get_bool("general.session.global_high_bit"))
      return TRUE;
  ...
  {
    strcpy(username,
           (*user->alias && ngcfg_get_bool("general.session.alias_system"))
              ? user->alias
              : user->name);
...
void Apply_Term_Caps(struct _usr *user)
{
  ...
  if (fp) {
    while (fgets(line, sizeof(line), fp)) {
      if (strncmp(line, "Telnet: ", 8) == 0)
        telnet = atoi(line + 8);
      else if (strncmp(line, "Ansi: ", 6) == 0)
        ansi = atoi(line + 6);
      else if (strncmp(line, "Rip: ", 5) == 0)
        rip = atoi(line + 5);
...
void Blank_User(struct _usr *user)
{
...

  {
    ...
    if (logon_priv==PREREGISTERED)
      user->priv=0;
    else
      user->priv=logon_priv;
  }
  ...
  if (ng_cfg)
  {
    ...
    Parse_Outside_Cmd((char *)ngcfg_get_string_raw("general.session.first_message_area"), temp);
    SetAreaName(user->msg, temp);

    Parse_Outside_Cmd((char *)ngcfg_get_string_raw("general.session.first_file_area"), temp);
...
void Find_Class_Number(void)
{
  ...
  if (cls==-1)
  {
    if (ngcfg_get_int("general.session.logon_priv") != PREREGISTERED || fLoggedOn)
    {
...
char * FileDateFormat(union stamp_combo *stamp,char *temp)
{
  ...
  ds = ngcfg_get_int("general.session.date_style");
...
word CharsetSwedish(byte *str, byte *ch)
{
  ...
  return (ngcfg_get_charset_int()==CHARSET_SWEDISH &&
          (*ch=='{' || *ch=='}' || *ch=='[' || *ch==']' ||
           *ch=='|' || *ch=='\\'));
}
...
void Check_Time_Limit(unsigned long *input_timeout, int *timer2)
{
  ...
  if (input_timeout && timeup(*input_timeout) &&
      (!local || (local && ngcfg_get_bool("maximus.local_input_timeout"))) &&
       !(fLoggedOn && acsflag(CFLAGA_NOTIME)))
  {
    if (*timer2)
    {
