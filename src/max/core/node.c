...
static char rcs_id[]="$Id: node.c,v 1.4 2004/01/28 06:38:11 paltas Exp $";
...
NFIND * NodeFindOpen(NETADDR *find)
{
  ...
  if (nlver==NLVER_5 || nlver==NLVER_6)
  {
    if (!V56FindOpen(nf))
    {
      free(nf);
...
static int near V56FindOpen(NFIND *nf)
{
  ...
  nf->v56.zone=ngcfg_get_matrix_primary_address().zone;
...
static NFIND * near V7FindOpen(NFIND *nf)
{
  ...
  if (V7FindNode(&tofind, &node6, netinfo) ||
      (tofind.point &&
       (tofind.point=0, V7FindNode(&tofind, &node6, netinfo))))
  {
...
static word near FDLookUpCost(struct _johofile *jf, struct _johonode *jn)
{
  ...
  sprintf(name, "%sPHONE.FDX", netinfo);
  ...
  sprintf(name, "%sPHONE.FDA", netinfo);
  ...
  else
  {
    ...
    if (!got_prefix)
    {
      ...
      prim=ngcfg_get_matrix_primary_address();
...
int NodeFindNext(NFIND *nf)
{
  ...
  if (nlver==NLVER_7 || nlver==NLVER_FD)
    return -1;
  ...
  if (read(v56->dfd, (nlver==5 ? (char *)&node5 : (char *)&node6),
