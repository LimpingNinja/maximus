...
static char rcs_id[]="$Id: f_logup.c,v 1.3 2004/01/27 21:00:28 paltas Exp $";
...
void Add_To_Upload_Log(char *path,char *fname,long bytes)
{
  ...
  if (*ngcfg_get_path("general.session.upload_log"))
  {
    strcpy(p1, (char *)ngcfg_get_path("general.session.upload_log"));
...
word LookForVirus(char *path, char *name)
{
  ...
  if (! *ngcfg_get_path("general.session.virus_check"))
    return FALSE;
