...
static char rcs_id[]="$Id: f_intrin.c,v 1.3 2004/01/27 21:00:28 paltas Exp $";
...
static int near ngcfg_get_protocol_bool(int protocol, const char *key)
{
  MaxCfgVar protos;
  MaxCfgVar item;
  MaxCfgVar v;
      ...
      maxcfg_var_count(&protos, &cnt) == MAXCFG_OK)
  {
    for (size_t i = 0; i < cnt; i++)
    {
      ...
      if (key && *key &&
          maxcfg_toml_table_get(&item, key, &v) == MAXCFG_OK &&
  ...
  if (protocol >= 0 && protocol < MAX_EXTERNP)    /* External protocol */
  {
        ...
        maxcfg_var_count(&protos, &cnt) == MAXCFG_OK)
    {
      for (size_t i = 0; i < cnt; i++)
      {
        ...
        if (maxcfg_toml_table_get(&item, "name", &v) == MAXCFG_OK && v.type == MAXCFG_VAR_STRING && v.v.s && *v.v.s)
          name = v.v.s;

        if (name && *name)
        {
          strnncpy(temp, (char *)name, PATHLEN-1);
          return temp;
        }
        break;
      }
...
sword IsBatch(sword protocol)
{
  if (protocol==PROTOCOL_XMODEM || protocol==PROTOCOL_XMODEM1K ||
      (protocol >= 0 && !ngcfg_get_protocol_bool(protocol, "batch")))
    return FALSE;
  else return TRUE;
}
