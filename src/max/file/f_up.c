...
static int g_upload_dupe_check_disable_depth = 0;
static int g_upload_dupe_ext_check_disable_depth = 0;

void upload_dupe_check_push(int disable_dupe, int disable_ext)
{
  if (disable_dupe)
    g_upload_dupe_check_disable_depth++;
  if (disable_ext)
    g_upload_dupe_ext_check_disable_depth++;
}

void upload_dupe_check_pop(int disable_dupe, int disable_ext)
{
  if (disable_dupe && g_upload_dupe_check_disable_depth > 0)
    g_upload_dupe_check_disable_depth--;
  if (disable_ext && g_upload_dupe_ext_check_disable_depth > 0)
    g_upload_dupe_ext_check_disable_depth--;
}
...
void File_Upload(char *mname)
{
...

  {
    ...
    if (min_free_kb && b_free < (unsigned long)(min_free_kb * 1000Lu))
  {
    Display_File(0, NULL, (char *)ngcfg_get_path("general.display_files.no_space"));
    return;
  }
...
void File_Get_Upload_Names(void)
{
  ...
  if (strlen(inp) > MAX_FN_LEN || strlen(testbuf) > 3 ||
      strpbrk(inp," |<>")!=NULL)
  {
    Display_File(0, NULL, (char *)ngcfg_get_path("general.display_files.fname_format"));
    return;
  }
  
  if (g_upload_dupe_check_disable_depth == 0 &&
...
word FileIsDupe(char *name)
{
  ...
  if (g_upload_dupe_ext_check_disable_depth > 0 ||
      !ngcfg_get_bool("general.session.upload_check_dupe_extension"))
  {
...
static word near FileIsBad(char *name)
{
  ...
  ret=FALSE;
  sprintf(badname, "%sbadfiles.bbs", (char *)ngcfg_get_path("maximus.sys_path"));
  ...
  if (protocol >= 0)
  {
    ...
    else
    {
      ...
      if (FindProtocol(protocol, pro))
      {
        ...
        if (pro->flag & P_ERL)
          sprintf(cmd, "%d %s", ngcfg_get_int("general.protocol.protoexit"), pro->ulcmd);
...
static void near FBLog(void)
{
  ...
  if (fexist(temp))
  {
    ...
    sprintf(temp, "runfb %s %s -u", (char *)ngcfg_get_path("maximus.file_data"), FAS(fah, name));
