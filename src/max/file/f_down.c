...
static char rcs_id[]="$Id: f_down.c,v 1.4 2004/01/27 21:00:28 paltas Exp $";
...
{
  const char *stage_base = ngcfg_get_path("maximus.stage_path");
  if (stage_base == NULL)
    stage_base = "";

  sprintf(szPath, "%snode%02x" PATH_DELIMS, stage_base, task_num);
}
...
{
  char szPath[PATHLEN];
  const char *szStagePath = ngcfg_get_path("maximus.stage_path");
  FFIND *ff;

  /* Make sure that the basic staging directory exists */

  if (szStagePath == NULL || *szStagePath==0)
    return FALSE;
    ...
    else
    {
      ...
      if (FindProtocol(protocol, pro))
      {
        ...
        else
        {
          ...
          if (pro->flag & P_ERL)
            sprintf(cmd, "%d %s", ngcfg_get_int("general.protocol.protoexit"), pro->dlcmd);
          else
