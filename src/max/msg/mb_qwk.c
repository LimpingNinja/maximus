...
static void near clean_attaches(int rcvd)
{
  ...
  if (rcvd)
    pdb=LFAdbOpen((char *)ngcfg_get_path("general.session.attach_base"));
  else
    pdb=NULL;
  for (q=qwk_atts; q; )
  {
...
int Make_QWK_Directory(void)
{
...

  {
    ...
    work_dir = ngcfg_get_path("general.reader.work_directory");
    packet_name = ngcfg_get_string("general.reader.packet_name");
    arc_ctl = ngcfg_get_path("general.reader.archivers_ctl");
    ...
    if (*work_dir=='\0' || *packet_name=='\0' || *arc_ctl=='\0')
      return -1;

    strnncpy(base_dir, (char *)work_dir, sizeof(base_dir));
    Add_Trailing(base_dir, PATH_DELIM);
    sprintf(qwk_path, "%snode%02x", base_dir, task_num);
  }
...
static int near Create_Control_DAT(void)
{
...

  {
    ...
    fprintf(cdat, ps_n, system_name ? system_name : "");
  }
  ...
  phone = ngcfg_get_string("general.reader.phone");
  if (phone == NULL)
    phone = "";
  fprintf(cdat, ps_n, phone);
  {
    const char *sysop = ngcfg_get_string_raw("maximus.sysop");
    fprintf(cdat, ps_n, sysop ? sysop : "");
  }
...
int QWKCallBack(char *line, void *args, int inbrowse)
{
  ...
  if (pqc->line_type & MSGLINE_SEENBY && !GEPriv(usr.priv, ngcfg_get_int("matrix.seenby_priv")))
    return 0;
  else if (pqc->line_type & MSGLINE_KLUDGE && !GEPriv(usr.priv, ngcfg_get_int("matrix.ctla_priv")))
    return 0;
...
static int near UpdateCounters(BROWSE *b)
{
  ...
  max_pack = ngcfg_get_int("general.reader.max_pack");
...
/*  Printf(xxx_msgs_packed, total_packed);*/
  ...
  if (packet_name == NULL)
    packet_name = "";
  {
    char packet_file[PATHLEN];
    snprintf(packet_file, sizeof(packet_file), "%s.qw%c", packet_name, qwk_ctr);
    if (safe_path_join(qwk_path, packet_file, qwkname, sizeof(qwkname)) != 0)
      qwkname[0] = '\0';
  }
  unlink(qwkname);
  ...
  {
    char wildcard[] = "*.*";
    if (safe_path_join(qwk_path, wildcard, files, sizeof(files)) != 0)
      files[0] = '\0';
  }
#else
  {
