...
static char rcs_id[]="$Id: m_upload.c,v 1.4 2004/01/28 06:38:10 paltas Exp $";
...
void Msg_Upload(void)
{
  ...
  *orig_msgid='\0';
  ...
  if (AddFileEntry(temp, 0, 0L)!=-1)
  {
    ...
    File_Get_Files(protocol, NULL, (char *)ngcfg_get_path("maximus.temp_path"));
  ...
  sprintf(temp, ss, (char *)ngcfg_get_path("maximus.temp_path"), fent.szName);
  ...
  else
  {
    ...
    else
    {
      ...
      {
        sprintf(temp, ss, (char *)ngcfg_get_path("maximus.temp_path"), No_Path(fent.szName));
        unlink(temp);
