...
static const char *near get_attach_base_path(void)
{
  return ngcfg_get_path("general.session.attach_base");
}

static const char *near get_attach_root_path(void)
{
  return ngcfg_get_path("general.session.attach_path");
}

static const char *near get_inbound_root_path(void)
{
  return ngcfg_get_path("maximus.inbound_path");
}

static const char *near get_user_file_path(void)
{
  return ngcfg_get_path("maximus.file_password");
}

static const char *near get_attach_archiver_name(void)
{
  return ngcfg_get_string("general.session.attach_archiver");
}
...
static int near Make_Attach_Directory(void)
{
  ...
  sprintf(attach_path,"%snode%02x", get_attach_root_path(), task_num);
...
static int near Receive_Attach(void)
{
  ...
  else
  {
    ...
    {
      unsigned long min_free_kb = (unsigned long)ngcfg_get_int("general.session.min_free_kb");
      if (min_free_kb && b_free < (long)min_free_kb*1000L)
      {
        Display_File(0, NULL, ngcfg_get_path("general.display_files.no_space"));
        return -1;
      }
...
static struct _arcinfo *near attach_archiver()
{
  ...
  {
    for (;ar;ar=ar->next)
      if (eqstri(get_attach_archiver_name(),ar->arcname))
        break;
    if (!ar)
      ar=ari;
  }
...
static int near Compress_Attach_Files(PMAH pmah, char *szAttachName, dword *pulFlags)
{
  ...
  else
  {
    ...
    if (!pmah->ma.attachpath)
      strcpy(pth, get_attach_root_path());
    else
    {
                                  ...
                                  dword ulFlags)
{
      ...
      else
      {
        if (ulFlags & LFA_FILE_PATH)
          *msg->subj = '\0';
          ...
          strcpy(msg->subj, get_attach_root_path());
        else
  ...
  else
  {
    ...
    DBASE OS2FAR * pDb = LFAdbOpen(get_attach_base_path());

    if (!pDb)
      logit(log_err_open_attach_db, get_attach_base_path());
    else
    {
      int ok;
      HUF huf;
      ...
      if ((huf=UserFileOpen((char *)get_user_file_path(), 0))!=NULL)
      {
        struct _usr u;
...
DBASE OS2FAR * Read_Attach(LFA_REC *plfa, XMSG *pxmsg, char * szCtrl, int isnetmsg)
{
  ...
  if (pxmsg && (pxmsg->attr & MSGFILE)!=0)
  {
        ...
        sscanf(p+6,"%lx %s\x01", &attachId, temp)==2)
    {
      ...
      if ((pdb=LFAdbOpen(get_attach_base_path()))==NULL)
        logit(log_err_open_attach_db, get_attach_base_path());
      else
      {
        int rc;
      ...
      for (s=strtok(pxmsg->subj,cmd_delim); s; s=strtok(NULL, cmd_delim))
      {
        ...
        if (isnetmsg || gotpath)
        {
          ...
          { /* Inbound files are deletable */
            strcpy(p, get_inbound_root_path());
            plfa->ulAttributes &= ~LFA_NODELETE;
          }
          else  /* Files in the attach directory are deletable */
            if (eqstrni(s, get_attach_root_path(), strlen(get_attach_root_path())))
              plfa->ulAttributes &= ~LFA_NODELETE;
          strcat(p, s);
          if (fexist(p))
          {
...
static int near Decompress_Attach(char *szName)
{
  ...
  else
  {
    ...
    {
      strcpy(szFullName, ngcfg_get_path("maximus.sys_path"));
      ...
      else
      {
        ...
        if (Make_Attach_Directory()!=-1)
        {
          ...
          if (FileEntries() &&
              ...
              ngcfg_get_kill_mode_int("general.session.kill_attach")==2 &&
...
int Msg_AttachedFiles(void)
{
  ...
  rc=FALSE;
  if (*ngcfg_get_string_raw("general.session.attach_base"))
  {
    DBASE OS2FAR * pdb;
          ...
          GetYnAnswer(attach_dload,0)!=NO)
      {
        ...
        else
        {
          ...
          if (Make_Attach_Directory()!=-1)
          {
            ...
            {
              if ((ngcfg_get_kill_mode_int("general.session.kill_attach")==2 && !(pLfaF->lfa.ulAttributes & LFA_NODELETEATTACH)) ||
                  Ask_KillAttach())
              {
                void *pvLookId[] = { NULL, NULL, NULL };
