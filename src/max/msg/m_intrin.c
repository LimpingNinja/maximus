...
static char rcs_id[]="$Id: m_intrin.c,v 1.5 2004/01/28 06:38:10 paltas Exp $";
...
{
  size_t cnt = ngcfg_get_matrix_address_count();
  ...
  {
    NETADDR n = ngcfg_get_matrix_address(i);
...
word CanSeeMsg(XMSG *msg)
{
  return ((msg->attr & MSGPRIVATE)==0 || ToOrFromUs(msg) ||
          GEPriv(usr.priv, ngcfg_get_int("matrix.private_priv")) || mailflag(CFLAGM_PVT));
}
