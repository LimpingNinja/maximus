...
static char rcs_id[]="$Id: m_for.c,v 1.4 2004/01/27 21:00:31 paltas Exp $";
    ...
    while (*word)
    {
      if (! *msg->to)
      {
        ...
        if (mah.ma.attribs & MA_NET)
          Get_FidoList_Name(msg, netnode, (char *)ngcfg_get_string_raw("matrix.fidouser"));
      }
...
static int near Fwd_Get_Parms(struct _fwdp *f)
{
  ...
  *temp='\0';
  ...
  if (f->bomb && !(GEPriv(realpriv(), ngcfg_get_int("matrix.fromfile_priv")) && local))
  {
