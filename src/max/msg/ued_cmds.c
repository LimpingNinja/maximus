...
static char rcs_id[]="$Id: ued_cmds.c,v 1.5 2004/01/28 06:38:11 paltas Exp $";
...
void UedFindFirstUser(void)
{
  ...
  {
    logit(cantread, (char *)ngcfg_get_path("maximus.file_password"));
    quit(ERROR_FILE);
  }
...
int UedPurgeUsers(void)
{
  ...
  /* Apply deletions to SQLite DB */
  if (pn > 0)
  {
    int i;
    for (i = 0; i < pn; i++)
    {
      (void)maxdb_user_delete((MaxDB *)huf->db, delids[i]);
    }
  }
