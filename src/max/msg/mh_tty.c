...
static char rcs_id[]="$Id: mh_tty.c,v 1.4 2004/01/28 06:38:11 paltas Exp $";
...
#define TTYQS 6
  ...
  if ((usr.help != EXPERT) && *ngcfg_get_string_raw("general.display_files.header_help"))
  {
    Puts(GRAY "\n");
    Display_File(DISPLAY_NONE, NULL, ngcfg_get_path("general.display_files.header_help"));
    Puts("\n");
  }
...
static word near ngcfg_get_msg_ask_priv(int idx)
{
  switch (idx)
  {
    case MSGKEY_PRIVATE:
      return ngcfg_get_int("matrix.message_edit.ask.private");
    case MSGKEY_CRASH:
      return ngcfg_get_int("matrix.message_edit.ask.crash");
    case MSGKEY_FILE:
      return ngcfg_get_int("matrix.message_edit.ask.fileattach");
    case MSGKEY_KILL:
      return ngcfg_get_int("matrix.message_edit.ask.killsent");
    case MSGKEY_HOLD:
      return ngcfg_get_int("matrix.message_edit.ask.hold");
    case MSGKEY_FRQ:
      return ngcfg_get_int("matrix.message_edit.ask.filerequest");
    case MSGKEY_URQ:
      return ngcfg_get_int("matrix.message_edit.ask.updaterequest");
    default:
      return 0;
  }
}

static word near ngcfg_get_msg_assume_priv(int idx)
{
  switch (idx)
  {
    case MSGKEY_PRIVATE:
      return ngcfg_get_int("matrix.message_edit.assume.private");
    case MSGKEY_CRASH:
      return ngcfg_get_int("matrix.message_edit.assume.crash");
    case MSGKEY_FILE:
      return ngcfg_get_int("matrix.message_edit.assume.fileattach");
    case MSGKEY_KILL:
      return ngcfg_get_int("matrix.message_edit.assume.killsent");
    case MSGKEY_HOLD:
      return ngcfg_get_int("matrix.message_edit.assume.hold");
    case MSGKEY_FRQ:
      return ngcfg_get_int("matrix.message_edit.assume.filerequest");
    case MSGKEY_URQ:
      return ngcfg_get_int("matrix.message_edit.assume.updaterequest");
    default:
      return 0;
  }
}
...
int near TTYGetNetmailAddress(PMAH pmah, XMSG *msg)
{
  ...
  do
  {
    ...
    else
    {
      ...
      if (GEPriv(usr.priv, ngcfg_get_int("matrix.unlisted_priv")))
        break;
      else
      {
...
int near TTYGetToField(PMAH pmah, XMSG *msg)
{
  ...
  {
    if (!ngcfg_get_bool("general.session.disable_userlist") && (pmah->ma.attribs & MA_NET)==0)
...
static void near DisplayEmptyHeader(PMAH pmah)
{
  ...
  else if (usr.video)
  {
    ...
    Printf("\x16\x19\x03"   "\n" CLEOL   "%c",
           TermLength() - !!(!local && ngcfg_get_bool("maximus.status_line")));
  }
