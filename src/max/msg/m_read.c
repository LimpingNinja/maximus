...
static char rcs_id[]="$Id: m_read.c,v 1.4 2004/01/28 06:38:10 paltas Exp $";
...
int ShowKludgeLines(char *ctrl, int (*showfunc)(char*,void*,int), void *args, int inbrowse)
{
  ...
  if (! GEPriv(usr.priv, ngcfg_get_int("matrix.ctla_priv")))
    return 0;
...
static int near Ask_KillMsg(XMSG *msg)
{
  ...
  kill_private = ngcfg_get_kill_mode_int("general.session.kill_private");
...
int Ask_KillAttach(void)
{
  ...
  kill_attach = ngcfg_get_kill_mode_int("general.session.kill_attach");
  kill_attach_priv = ngcfg_get_int("general.session.kill_attach_priv");
  ...
  switch (kill_attach)
  {
    ...
    case 1:               /* Ask if we should kill attach */
      ...
      if (!GEPriv(usr.priv, kill_attach_priv))
        rc=TRUE;
      else
      {
...
static void near Show_Replies(XMSG *msg)
{
  ...
  if (msg->replyto)
  {
    use_umsgids = ngcfg_get_bool("general.session.use_umsgids");
    otlong=use_umsgids ? msg->replyto
  ...
  {
    use_umsgids = ngcfg_get_bool("general.session.use_umsgids");
    tlong=use_umsgids ? msg->replies[0] : MsgUidToMsgn(sq, msg->replies[0], UID_EXACT);
    ...
    if (tlong)
    {
      ...
      {
        use_umsgids = ngcfg_get_bool("general.session.use_umsgids");
        tlong=use_umsgids ? msg->replies[i] :
...
static int near NoMoreMessage(word this, word got, byte *outline[], byte lt[])
{
  ...
  {
    if (*outline[i] ||
        ((lt[i] & MSGLINE_SEENBY) && GEPriv(usr.priv, ngcfg_get_int("matrix.seenby_priv"))) ||
        ((lt[i] & MSGLINE_KLUDGE) && GEPriv(usr.priv, ngcfg_get_int("matrix.ctla_priv"))))
    {
      return FALSE;
    }
  }
...
static int near CanShowLine(char linetype)
{
  ...
  {
    if (! GEPriv(usr.priv, ngcfg_get_int("matrix.seenby_priv")))
      return FALSE;
  }
