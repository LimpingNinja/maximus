...
static char rcs_id[]="$Id: mh_graph.c,v 1.4 2004/01/28 06:38:11 paltas Exp $";
      ...
      switch (i)
      {
        case MSGKEY_PRIVATE:
          if ((pmah->ma.attribs & (MA_PVT|MA_PUB)) == (MA_PVT|MA_PUB))
            msg_ask_priv = usr.priv;
          break;
        default:
          msg_ask_priv = ngcfg_get_msg_ask_priv(i);
          break;
      }

      if (GEPriv(usr.priv, msg_ask_priv) || mailflag(CFLAGM_ATTRANY))
        break;
    }
...
int near GetFromAddr(PMAH pmah, XMSG *msg)
{
  ...
  if (*tempaddr=='\0')
  {
    NETADDR prim = ngcfg_get_matrix_primary_address();
    strcpy(tempaddr, Address(&prim));
  }
...
int near GetToField(PMAH pmah, XMSG *msg)
{
  ...
  if (usr.help != EXPERT && !ngcfg_get_bool("general.session.disable_userlist") &&
  ...
  if ((*netnode=='\0' || !eqstri(first_name, msg->to)) &&
      (pmah->ma.attribs & MA_NET))
  {
    if (Get_FidoList_Name(msg, netnode, (char *)ngcfg_get_string_raw("matrix.fidouser")))
...
static void near RedrawAll(PMAH pmah, XMSG *msg, int showhelp)
{
  ...
  if (showhelp && (usr.help != EXPERT) && *ngcfg_get_string_raw("general.display_files.header_help"))
  {
    Puts(GRAY "\n");
