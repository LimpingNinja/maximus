...
static char rcs_id[]="$Id: m_editor.c,v 1.3 2004/01/27 21:00:31 paltas Exp $";
...
int Editor(XMSG *msg, HMSG msgh, long msgnum, char *ctrl_buf, struct _replyp *pr)
{
  ...
  if (*ngcfg_get_string_raw("general.display_files.entry_help"))
    Display_File(0, NULL, ngcfg_get_path("general.display_files.entry_help"));

  if (*ngcfg_get_string_raw("general.session.local_editor"))
  {
    if (local || *ngcfg_get_string_raw("general.session.local_editor")=='@')
    {
  ...
  else if ((usr.bits2 & BITS2_BORED) || ngcfg_get_bool("general.session.disable_magnet"))
  {
    Puts(CLEOS);
