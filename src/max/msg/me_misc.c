...
static char rcs_id[]="$Id: me_misc.c,v 1.5 2004/01/28 06:38:10 paltas Exp $";
...
void GenerateOriginLine(char *text, PMAH pmah)
{
  ...
  sprintf(text, tearline,
          ...
          /* origin */
          pmah->ma.origin ? PMAS(pmah, origin) : ngcfg_get_string_raw("maximus.system_name"),
...
static int near Handle_Matrix_Charges(NETADDR *dest,int total)
{
  ...
  if ((nf=NodeFindOpen(dest))==NULL)
  {
    ...
    cost=(word)ngcfg_get_int("matrix.unlisted_cost");
  }
  else
  {
    cost=nf->found.cost;
...
char * GenerateMessageKludges(XMSG *msg, PMAH pmah, char *ctrl_buf)
{
  ...
  *kludge='\0';
  ...
  if (ngcfg_get_bool("general.session.gate_netmail") && msg->dest.zone != msg->orig.zone &&
...
void AddToEchoToss(char *tag)
{
  ...
  int found=FALSE;
  FILE *fptr;
  const char *echotoss_name = ngcfg_get_string_raw("matrix.echotoss_name");

  if (!ngcfg_get_bool("matrix.log_echomail"))
    return;

  if (strchr(echotoss_name, '%'))
    Parse_Outside_Cmd((char *)echotoss_name, temp);
  else strcpy(temp, (char *)echotoss_name);
