...
static char rcs_id[]="$Id: m_header.c,v 1.5 2004/01/28 06:38:10 paltas Exp $";
...
int CheckCredit(NETADDR *dest, PMAH pmah)
{
  ...
  if ((nf=NodeFindOpen(dest))==NULL)
  {
    ...
    cost=ngcfg_get_int("matrix.unlisted_cost");
  }
  else
  {
    cost=nf->found.cost;
...
int Get_FidoList_Name(XMSG *msg, char *netnode, char *fidouser)
{
  ...
  {
    const char *net_info_base = ngcfg_get_path("maximus.net_info_path");
    int nlver = ngcfg_get_nodelist_version_int();
    ...
    if (nlver==NLVER_7)
    {
      ...
      if (V7FindName(msg->to, &dest, &node, net_info_base))
      {
  ...
  if (nlver==NLVER_FD)
  {
    ...
    if (FDFindName(name, &dest, net_info_base))
    {
      strcpy(netnode, Address(&dest));
      return TRUE;
    }
...
#ifndef UNIX
  sprintf(temp, "%sNAMES.MAX", (char *)ngcfg_get_path("maximus.sys_path"));
#else
  sprintf(temp, "%snames.max", (char *)ngcfg_get_path("maximus.sys_path"));
#endif
