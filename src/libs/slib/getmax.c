...
/* Obtain the Maximus install/config root directory, either through a
 * command-line option or via environment variables.
 */
...
char *GetMaximus(int argc, char **argv, int iBadErrorlevel)
{
  ...
  if (!pszMaximus)
    pszMaximus = getenv("MAX_INSTALL_PATH");
  if (!pszMaximus)
    pszMaximus = getenv("MAXIMUS");

  if (!pszMaximus)
  {
    printf("Error!  The 'MAXIMUS' environment variable could not be found.\n"
           "This variable must point to your Maximus install/config root.\n\n"
