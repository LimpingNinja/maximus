...
1. **[MEX Compiler]({{ site.baseurl }}{% link mex-compiler.md %})** — What the compiler does,
   how to invoke it, and what the output looks like.
2. **[Script Structure]({{ site.baseurl }}{% link mex-script-structure.md %})** — The anatomy
   of a `.mex` file: includes, functions, `main()`, and how it all fits
   together.
3. **[Hello World]({{ site.baseurl }}{% link mex-hello-world.md %})** — Write it, compile it,
   wire it to a menu option, and watch it run.

After that, you'll know enough to be dangerous. Which is exactly where
**[Learning MEX]({{ site.baseurl }}{% link mex-learning.md %})** picks up — a hands-on,
...
happy caller. (Details are in the [Menu Options]({{ site.baseurl }}{% link config-menu-options.md %})
reference under the `MEX` command.)

## Ready?

Pick up at the [MEX Compiler]({{ site.baseurl }}{% link mex-compiler.md %}) if you want the
methodical walk-through, or jump straight to
[Learning MEX]({{ site.baseurl }}{% link mex-learning.md %}) if you learn by doing and
